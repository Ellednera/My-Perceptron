# This Makefile is for the My::Perceptron extension to perl.
#
...
#     NAME => q[My::Perceptron]
#     PREREQ_PM => { FindBin=>q[0], Storable=>q[3.15], Test::More=>q[0], Text::CSV=>q[2.01] }
#     TEST_REQUIRES => { FindBin=>q[0], Test::More=>q[0] }
#     VERSION_FROM => q[lib/My/Perceptron.pm]
#     clean => { FILES=>q[My-Perceptron-*] }
...
MAN3SECTION = 3
INSTALLDIRS = site
INSTALL_BASE = /home/raphael/perl5
DESTDIR = 
PREFIX = $(INSTALL_BASE)
...
MAKEMAKER   = /usr/local/share/perl/5.30.0/ExtUtils/MakeMaker.pm
MM_VERSION  = 7.62
MM_REVISION = 76200
...
MAKE = make
FULLEXT = My/Perceptron
BASEEXT = Perceptron
PARENT_NAME = My
DLBASE = $(BASEEXT)
VERSION_FROM = lib/My/Perceptron.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/My/Perceptron.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIBDEP)$(DFSEP)Config.pm $(PERL_INCDEP)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/My
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/My
...
TO_INST_PM = lib/My/Perceptron.pm
...
DISTNAME = My-Perceptron
DISTVNAME = My-Perceptron-0.01
...
manifypods : pure_all config  \
	lib/My/Perceptron.pm
	$(NOECHO) $(POD2MAN) --section=$(MAN3SECTION) --perm_rw=$(PERM_RW) -u \
	  lib/My/Perceptron.pm $(INST_MAN3DIR)/My::Perceptron.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name: My-Perceptron' >> META_new.yml
	$(NOECHO) $(ECHO) 'no_index:' >> META_new.yml
	$(NOECHO) $(ECHO) '  directory:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - t' >> META_new.yml
	$(NOECHO) $(ECHO) '    - inc' >> META_new.yml
	$(NOECHO) $(ECHO) 'requires:' >> META_new.yml
	$(NOECHO) $(ECHO) '  Storable: '\''3.15'\''' >> META_new.yml
	$(NOECHO) $(ECHO) '  Text::CSV: '\''2.01'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "My-Perceptron",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '         "requires" : {' >> META_new.json
	$(NOECHO) $(ECHO) '            "Storable" : "3.15",' >> META_new.json
	$(NOECHO) $(ECHO) '            "Text::CSV" : "2.01",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="My-Perceptron" VERSION="0.01">' > My-Perceptron.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> My-Perceptron.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Raphael Jong Jun Jie &lt;ellednera@cpan.org&gt;</AUTHOR>' >> My-Perceptron.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> My-Perceptron.ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,006,0,0" />' >> My-Perceptron.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Storable::" VERSION="3.15" />' >> My-Perceptron.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Text::CSV" VERSION="2.01" />' >> My-Perceptron.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-gnu-thread-multi-5.30" />' >> My-Perceptron.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> My-Perceptron.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> My-Perceptron.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> My-Perceptron.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'lib/My/Perceptron.pm' 'blib/lib/My/Perceptron.pm' 
